...
Функция ПолучитьРасшифровкуОтчета()
	...
	Запрос.Текст = 
		...
		|	ОстаткиПоДушниламОстатки.БаллОстаток КАК Балл
		|ИЗ
		|	РегистрНакопления.ОстаткиПоДушнилам.Остатки(&КонецСвета) КАК ОстаткиПоДушниламОстатки
		|ГДЕ
		|	ОстаткиПоДушниламОстатки.БаллОстаток < 0";
	Запрос.УстановитьПараметр("КонецСвета", КонецСвета);
	РезультатЗапроса = Запрос.Выполнить();
...
Функция ПолучитьОтветНаСообщение(Тег)
	...
	Если НРег(Тег) = "/инфо" Тогда
		...
		ОтветноеСообщение = ВыслатьИнфо();
		ОтветноеСообщение = ОбрамитьВТегHTML(ОтветноеСообщение, "<code>");
		
		Возврат СтрСоединить(ОтветноеСообщение, Символы.ПС);
...
Функция ТестВыслатьИнфо()
	...
	ТестНеПройден = Истина;
	
	Возврат ТестНеПройден;
